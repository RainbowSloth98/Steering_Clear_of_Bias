...
#* Block 2 - Loading in a custom SAE , comment out potentially.
#region
...
print("STOP")
    ...
    #? In our sae object we need to have a CustomSAEConfig.
    #? This contains some information which is used by the evals (hook_name, hook_layer, model_name, d_sae, etc).
    #? In addition, it contains information that is used by our plotting functions, like number of training tokens and architecture.
    #? For example, we should have the sae.cfg.architecture defined if we want to plot multiple SAE architectures.
    #? Note: Everything in this cell, except architecture and training_tokens, is done in the BaseSAE class that the ReluSAE inherits from.
    #? Because of this, we recommend that you modify an existing SAE class.
...
print("STOP")
    ...
    #? `selected_saes` is a list of tuples of (unique_sae_id, sae object) OR (sae lens release, sae lens id). 
    #? If it is a list of custom sae objects, then memory size will increase with the length of the list. 
    #? This is especially important if the SAEs are large. 
    #? If memory is a concern, I recommend calling the `run_eval()` function multiple times with lists of length 1, 
    #?     each list containing a new sae object.
...
print("STOP")
    ...
    #? Select your baseline SAEs here. 
    #? Refer to `sae_regex_selection.ipynb` for more regex patterns. 
    #? We are going to get a topk SAE from the same layer.
...
print("STOP")
    ...
    #? Now we will run all of the evals, and create more plots. 
    #! Be warned - this takes around an hour. 
    #? Note that a significant amount of the costs are one time per layer - 
    #*     for example, with absorption, we have to train a bunch of probes on a given layer. 
    #? So, if we have multiple SAEs per layer the cost should be much less than 30 minutes per SAE. 
    #? In addition, to save disk space usage we currently are not saving activations for reuse by multiple SAEs.

    #? Additionally, we can make this faster by evaluating on a subset of the datasets. 
    #? Sparse probing and Spurious Correlation Removal both have approximately 8 datasets each. 
    #? We could lower the time by using fewer datasets at the cost of not having as strong of a signal.
